# 6_ 게임 오버 처리
#  # 1 . 모든 공을 없애거나, 2. 캐릭터가 공에 닿았거나, 제한 시간 초과시
...
game_result = 'Game Over'  # 게임종료 메시지
...
# 무기와 공이 닿았을 시,  사라질 무기, 공 정보 저장을 위한 변수
weapon_remove = -1
ball_remove = -1
...
# 즉 x는 그대로 이고 y 좌표만 스피드만큼 뺴주는 것
    ...
    # 공의 경계값 처리
        ...
        # 4. 충돌 처리
        # (2가지 충돌 예상) : 캐릭터와 공이 닿았을 경우, 무기와 공이 닿았을 경우
        # 캐릭터 rect 정보 저장 후 업데이트
        ...
        for ball_idx, ball_val in enumerate(balls):
            ...
            for weapon_idx, weapon_val in enumerate(weapons):
                ...
                if weapon_rect.colliderect(ball_rect):
                    weapon_remove = weapon_idx  # 해당 무기 삭제위한 값 설정
                    ball_remove = ball_idx  # 해당 공 삭제위한 값 설정

                    # 가장 작은 볼이 아니라면 다음 단계의 크기로 나눠주는 처리
                    if ball_img_idx < 3:
                        # 현재 공 크기 정보를 가져옴
                    ...
                    break
            # else:  # 계속 게임 진행
            #     continue  # 안쪽 for문 조건이 맞지 않다면 바깥쪽 for문 계속 수행
            # break  # 안쪽 for문에서 break를 만나면 진입, 2중 for문 한번에 탈출

            # 충돌된 공 or 무기 삭제
            if ball_remove > -1:  # 인덱스값은 0,1,2 ...으로 가기 떄문에
                del balls[ball_remove]
                ball_remove = -1  # 다음 프레임에서 똑같이 -1로 시작하고 업데이트될 때 처리
            if weapon_remove > -1:
...
pygame.display.update()  # 업데이트 해주기 화면 그리기는 계속 해줘야 함
pygame.time.delay(2000)  # 딜레이를 안걸면 바로 종료 되기 떄문에 딜레이 추가

pygame.quit()


# 다시 한번 복습 후 40분짜리 게임 => 웹스크롤링 빠르게 정독 후 react 로 넘어가기
